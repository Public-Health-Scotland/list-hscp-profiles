...
other_loc_names <- paste(other_locs$hscp_locality, sep = "\n") |>
...
make_summary_table <- function(data) {
  ...
  data |>
    mutate(
      ...
      across(all_of(c(other_loc_cols, hscp_col, scot_col)), str_squish),
      # Make any missing values a dash
      across(all_of(c(other_loc_cols, hscp_col, scot_col)), \(figure_num) na_if(figure_num, "NA")),
      across(all_of(c(other_loc_cols, hscp_col, scot_col)), \(figure_num) replace_na(figure_num, "-"))
    ) |>
    flextable() |>
    # Header
...
demographic_values <- tibble(
  indicators, datatype, time, other_loc_values,
  hscp_values, scot_values
)

colnames(demographic_values) <- c("Indicators", headings)
...
colnames(population_health_values) <- c("Population Health Indicators", headings)

population_health_table <- make_summary_table(population_health_values)

population_health_table
